...
import { ENV, OUTBOX_WORKER_LEASE_MIN } from "../../type/Env";
...
test("must work correctly (failed flow < max retries)", async () => {
    await externalHandler.createOrder(createOrder, idempotencyKey);
...
test("must work correctly (failed flow < max retries) and process a failed event", async () => {
    await externalHandler.createOrder(createOrder, idempotencyKey);
