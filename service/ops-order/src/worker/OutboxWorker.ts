...
export class OutboxWorker {
    ...
    registerWorker(): void {
        const interval = setInterval(() => this.processWork(), OUTBOX_WORKER_CYCLE_DELAY_MIN * 60 * 1000);
    ...
    async processWork(): Promise<void> {
        ...
        console.log(`[OutboxWorker] --- Outbox worker cycle [${this.cycle++}] on PPID [${process.ppid}] has successfully published [${selectedOutboxRecords.length}] records. Time: ${new Date()}`);
    }
}
